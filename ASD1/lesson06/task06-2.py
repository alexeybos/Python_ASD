...
class MinDeque(Deque):
    ...
    def get_min(self):
        if self.min_tail._size() == 0 and self.min_head._size() == 0:
            return None
        elif self.min_tail._size() == 0:
            return self.min_head.peek()
        elif self.min_head._size() == 0:
    ...
    def remove_min(self, val, stack1: Stack, stack2: Stack):
        if stack1._size() > 0 and stack1.peek() == val:
            return stack1.pop()
        while stack2._size() > 0 and stack2.peek() >= val:
            stack1.push(stack2.pop())
        return stack1.pop()

    def addFront(self, item):
        if self.min_head._size() == 0 or item <= self.min_head.peek():
            self.min_head.push(item)
        super().addFront(item)

    def addTail(self, item):
        if self.min_tail._size() == 0 or item <= self.min_tail.peek():
